...
// This makes it easy to map through your product data
  ...
  return (
    <div className="group flex flex-col gap-3">
      ...
      <div className="relative aspect-[3/4] w-full overflow-hidden rounded-md bg-gray-100">
        ...
        {/* Badges (New, Best Seller, etc.) */}
        {product.badge && (
          <div className={`absolute top-3 left-3 z-10 rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center gap-2">
          ...
          {product.oldPrice && (
            <span className="text-sm text-slate-400 line-through">S/ {product.oldPrice}</span>
          )}
        </div>
...
export default function CatalogoPage() {
  ...
  // State for the price slider (example)
  const [maxPrice, setMaxPrice] = useState(300);

  // Example Product Data based on the screenshots
  const products = [
    {
      id: 1,
      ...
      image: 'https://images.unsplash.com/photo-1529139574466-a303027c028b?q=80&w=600&auto=format&fit=crop'
    },
    {
      id: 2,
  ...
  return (
    <div className="w-full bg-[#f8f9fa] min-h-screen font-sans pb-24">
      ...
      {/* 1. HEADER SECTION */}
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12 pt-12 pb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl font-bold text-slate-900 tracking-tight mb-1">Catálogo</h1>
      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12">
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <aside className="w-full lg:w-64 flex-shrink-0 space-y-10">
            ...
            {/* Categories */}
            <div>
              <h3 className="mb-4 text-xs font-bold uppercase tracking-widest text-slate-900">Categoría</h3>
              <ul className="space-y-2">
                <li>
                  <a href="#" className="block rounded-md bg-[#0B101B] px-4 py-2.5 text-sm font-medium text-white transition-colors">Todos</a>
                </li>
                <li>
                  <a href="#" className="block rounded-md px-4 py-2 text-sm text-slate-600 hover:bg-slate-200 transition-colors">Polos</a>
                </li>
                <li>
                  <a href="#" className="block rounded-md px-4 py-2 text-sm text-slate-600 hover:bg-slate-200 transition-colors">Hoodies</a>
                </li>
                <li>
                  <a href="#" className="block rounded-md px-4 py-2 text-sm text-slate-600 hover:bg-slate-200 transition-colors">Pantalones</a>
                </li>
                <li>
                  <a href="#" className="block rounded-md px-4 py-2 text-sm text-slate-600 hover:bg-slate-200 transition-colors">Chaquetas</a>
                </li>
                <li>
                  <a href="#" className="block rounded-md px-4 py-2 text-sm text-slate-600 hover:bg-slate-200 transition-colors">Shorts</a>
                </li>
              </ul>
            </div>
            ...
            {/* Sizes */}
            <div>
              <h3 className="mb-4 text-xs font-bold uppercase tracking-widest text-slate-900">Talla</h3>
              <div className="grid grid-cols-4 gap-2">
                {['XS', 'S', 'M', 'L', 'XL', '2XL'].map((size) => (
                  <button key={size} className="flex h-10 items-center justify-center rounded border border-gray-300 bg-white text-sm text-slate-600 transition-colors hover:border-slate-400 hover:bg-slate-50 focus:border-slate-900 focus:ring-1 focus:ring-slate-900">
                    {size}
                  </button>
                ))}
              </div>
            ...
            {/* Colors */}
            <div>
              <h3 className="mb-4 text-xs font-bold uppercase tracking-widest text-slate-900">Color</h3>
              <div className="flex flex-wrap gap-3">
                {['#000000', '#ffffff', '#6b705c', '#c65d3b', '#d8c8b8', '#1c2841'].map((color, idx) => (
                  <button 
                    key={idx} 
            ...
            <div>
               <div className="mb-4 flex items-center justify-between">
                <h3 className="text-xs font-bold uppercase tracking-widest text-slate-900">Precio Máximo:</h3>
                <span className="text-sm font-semibold text-blue-600">S/ {maxPrice}</span>
              </div>
